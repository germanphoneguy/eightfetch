...
impl Colors {
    ...
    fn custom(hex: &str) -> Option<Self> {
        ...
        // Leak the strings for &'static str
...
fn get_gpu_from_sysfs() -> Vec<(String, String)> {
    ...
    // Approach 1: Check GPU driver directories (fast — only loaded drivers)
    ...
    // Approach 2: Full PCI device scan (fallback — slower but works without drivers)
    if gpus.is_empty() {
...
fn get_gpu() -> (String, String) {
    ...
    // Fallback to lspci if sysfs returned nothing
    if gpus.is_empty() {
...
fn get_wm_de() -> (String, String) {
    ...
    // WM detection via env vars (fastest, no subprocess)
    ...
    // Fallback: check process list for common WMs
    if wm.is_empty() {
    ...
    if de.is_empty() && wm.is_empty() {
        // Try reading .xinitrc or similar
        if let Ok(home) = env::var("HOME") {
            if Path::new(&format!("{}/.xinitrc", home)).exists() {
                // Could parse, but skip for speed
            }
        }
    }
...
fn get_disk_usage() -> String {
    // Use statvfs directly instead of spawning df
